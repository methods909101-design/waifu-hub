-- WaifuHub Supabase Database Schema - Fixed RLS Policies
...
-- FIXED: These policies now work with anon key and don't require auth.uid()
...
-- FIXED RLS POLICIES: Allow all operations for anon users (API will handle authorization)
-- This is necessary because we're using wallet-based auth, not Supabase Auth

-- Users policies - allow all operations
CREATE POLICY "Allow all operations on users" ON users FOR ALL USING (true) WITH CHECK (true);

-- Waifus policies - allow all operations
CREATE POLICY "Allow all operations on waifus" ON waifus FOR ALL USING (true) WITH CHECK (true);

-- Votes policies - allow all operations
