-- WaifuHub Supabase Database Schema (Safe Version)
-- This file handles existing tables and only creates what's missing
-- Run this in your Supabase SQL editor to safely update the database
...
BEGIN
    ...
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'last_waifu_creation') THEN
        ALTER TABLE users ADD COLUMN last_waifu_creation TIMESTAMP WITH TIME ZONE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'users' AND column_name = 'last_vote') THEN
        ALTER TABLE users ADD COLUMN last_vote TIMESTAMP WITH TIME ZONE;
    END IF;
    ...
    -- Add columns to waifus table if they don't exist
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'waifus' AND column_name = 'hair_color') THEN
        ALTER TABLE waifus ADD COLUMN hair_color TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'waifus' AND column_name = 'character_profile') THEN
        ALTER TABLE waifus ADD COLUMN character_profile JSONB;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'waifus' AND column_name = 'video_url') THEN
        ALTER TABLE waifus ADD COLUMN video_url TEXT;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'waifus' AND column_name = 'is_published') THEN
        ALTER TABLE waifus ADD COLUMN is_published BOOLEAN DEFAULT FALSE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'waifus' AND column_name = 'likes') THEN
        ALTER TABLE waifus ADD COLUMN likes INTEGER DEFAULT 0;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'waifus' AND column_name = 'dislikes') THEN
        ALTER TABLE waifus ADD COLUMN dislikes INTEGER DEFAULT 0;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'waifus' AND column_name = 'published_at') THEN
        ALTER TABLE waifus ADD COLUMN published_at TIMESTAMP WITH TIME ZONE;
    END IF;
    
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'waifus' AND column_name = 'updated_at') THEN
        ALTER TABLE waifus ADD COLUMN updated_at TIMESTAMP WITH TIME ZONE DEFAULT NOW();
    END IF;
END $$;

-- Create indexes for better performance (only if they don't exist)
CREATE INDEX IF NOT EXISTS idx_users_wallet_address ON users(wallet_address);
CREATE INDEX IF NOT EXISTS idx_users_last_waifu_creation ON users(last_waifu_creation);
CREATE INDEX IF NOT EXISTS idx_users_last_vote ON users(last_vote);
CREATE INDEX IF NOT EXISTS idx_waifus_user_id ON waifus(user_id);
CREATE INDEX IF NOT EXISTS idx_waifus_is_published ON waifus(is_published);
CREATE INDEX IF NOT EXISTS idx_waifus_created_at ON waifus(created_at);
CREATE INDEX IF NOT EXISTS idx_waifus_published_at ON waifus(published_at);
CREATE INDEX IF NOT EXISTS idx_waifus_likes ON waifus(likes);
CREATE INDEX IF NOT EXISTS idx_waifus_dislikes ON waifus(dislikes);
CREATE INDEX IF NOT EXISTS idx_votes_user_id ON votes(user_id);
CREATE INDEX IF NOT EXISTS idx_votes_waifu_id ON votes(waifu_id);
CREATE INDEX IF NOT EXISTS idx_votes_created_at ON votes(created_at);
CREATE INDEX IF NOT EXISTS idx_chat_messages_user_id ON chat_messages(user_id);
CREATE INDEX IF NOT EXISTS idx_chat_messages_waifu_id ON chat_messages(waifu_id);
...
-- Create triggers for automatic vote count updates (drop first if exists)
...
DROP TRIGGER IF EXISTS waifus_updated_at ON waifus;
...
DROP POLICY IF EXISTS "Anyone can view published waifus" ON waifus;
DROP POLICY IF EXISTS "Users can create their own waifus" ON waifus;
DROP POLICY IF EXISTS "Users can update their own waifus" ON waifus;
DROP POLICY IF EXISTS "Users can delete their own waifus" ON waifus;
...
DROP VIEW IF EXISTS published_waifus_with_creator;
DROP VIEW IF EXISTS user_stats;
DROP VIEW IF EXISTS waifu_leaderboard;
...
DO $$
BEGIN
    RAISE NOTICE 'WaifuHub database schema updated successfully!';
    RAISE NOTICE 'All tables, functions, triggers, and policies are now ready.';
    RAISE NOTICE 'You can now test the application with your Supabase database.';
END $$;
